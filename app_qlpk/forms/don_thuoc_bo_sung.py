...
class DonThuocKhac(QWidget):
    ...
    def initUI(self):
        ...
        grid.addWidget(self.chandoan, 3, 1, 1, 3)
        ...
        grid.addWidget(self.diung, 4, 1, 1, 3)
        ...
        lbl_ds.setStyleSheet("color: red; font-weight: bold;")
        ...
        try:
            ...
            if "Xuất thuốc" in self.btns:
                self.btns["Xuất thuốc"].clicked.connect(self.on_xuat_thuoc_bo_sung)
    ...
    def load_patients(self):
        ...
        try:
            ...
            for id, name, ngaysinh, gioitinh, tuoi, dienthoai, diachi, doituong, record in patients:
                display_name = f"{name} - {record}" if record else name
        ...
        try:
            for i in range(self.benhnhan.count()):
                ...
                if isinstance(data, dict) and data.get('id') == patient_id:
                    # block signals to avoid redundant reload
                    try:
                        self.benhnhan.blockSignals(True)
                    except Exception:
                        pass
                    try:
                        ...
                        try:
                            self.benhnhan.blockSignals(False)
                        except Exception:
                            pass
                    return
        except Exception:
    ...
    def on_xuat_thuoc_bo_sung(self):
        """Mark exported for the latest saved supplementary prescription."""
        conn = None
        try:
            conn = get_connection()
            cur = conn.cursor()
            ...
            row = cur.fetchone()
            if not row:
                QMessageBox.warning(self, "Cảnh báo", "Vui lòng lưu đơn trước khi xuất thuốc.")
                return

            don_id = row[0]

            # 2. Đọc chi tiết đơn thuốc - chọn cột FK phù hợp trước khi truy vấn
            medicines = []
            try:
                ...
                if fk_col:
                    ...
                    # Table or expected FK column missing; try both common names safely
                    try:
            ...
            # 3. Nếu không có chi tiết → chỉ đánh dấu xuất thuốc
            if not medicines:
                try:
                    cur.execute("PRAGMA table_info(don_thuoc_bo_sung)")
                    cols = [c[1] for c in cur.fetchall()]
                    if 'xuat_thuoc' not in cols:
                        cur.execute("ALTER TABLE don_thuoc_bo_sung ADD COLUMN xuat_thuoc INTEGER DEFAULT 0")

                    cur.execute("UPDATE don_thuoc_bo_sung SET xuat_thuoc = 1 WHERE id = ?", (don_id,))
                    conn.commit()
                    QMessageBox.information(self, "Hoàn tất", "Đã đánh dấu xuất thuốc cho đơn bổ sung.")
                    return

                except Exception as e:
                    conn.rollback()
                    QMessageBox.critical(self, "Lỗi", f"Không thể đánh dấu xuất thuốc: {e}")
                    return
            ...
            for ma_thuoc, ten_thuoc, so_luong in medicines:
                if not ma_thuoc:
                    continue  # thuốc khác không có mã
