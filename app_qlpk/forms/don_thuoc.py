...
class KeDonThuoc(QWidget):
    ...
    def initUI(self):
        ...
        try:
            ...
            pass
        btn_layout.addWidget(self.btn_nhapmoi)
        ...
        try:
            ...
            pass
        btn_layout.addWidget(self.btn_in_don)
        ...
        try:
            ...
            pass
        btn_layout.addWidget(self.btn_xuat_thuoc)
    ...
    def on_xuat_thuoc(self):
        """Xuất thuốc: kiểm tra tồn kho và trừ số lượng từ danh_muc_thuoc"""
        if not hasattr(self, 'last_don_thuoc_id') or not self.last_don_thuoc_id:
            QMessageBox.warning(self, "Cảnh báo", "Vui lòng lưu đơn thuốc trước khi xuất!")
            return
        ...
        try:
            ...
            # Get all medicines for this don_thuoc
            cur.execute("""
                SELECT ma_thuoc, ten_thuoc, so_luong
            ...
            if not medicines:
                QMessageBox.warning(self, "Cảnh báo", "Không có thuốc để xuất!")
                return
            
            # Check inventory and prepare update list
            insufficient = []
            updates = []
            
            for ma_thuoc, ten_thuoc, so_luong in medicines:
                cur.execute("""
                    SELECT ton_kho FROM danh_muc_thuoc WHERE ma_thuoc = ?
                """, (ma_thuoc,))
                result = cur.fetchone()
                ...
                ton_kho = result[0] if result[0] else 0
                
                if ton_kho < so_luong:
                    insufficient.append(f"{ten_thuoc} - Tồn kho: {ton_kho}, cần xuất: {so_luong}")
                else:
                    # Valid: add to update list
                    new_ton_kho = ton_kho - so_luong
                    updates.append((new_ton_kho, ma_thuoc))
            
            # If there are insufficient items, show warning
            if insufficient:
                msg = "Các thuốc không đủ tồn kho:\n" + "\n".join(insufficient)
                if not updates:
                    # No valid medicines to export
                    QMessageBox.warning(self, "Lỗi", msg)
                    conn.close()
                    return
                else:
                    # Ask if user wants to continue with available medicines
                    reply = QMessageBox.warning(self, "Cảnh báo", 
                        msg + "\n\nBạn có muốn tiếp tục xuất những thuốc có đủ tồn kho không?",
            ...
            try:
                ...
                for new_ton_kho, ma_thuoc in updates:
                    cur.execute("""
                        UPDATE danh_muc_thuoc SET ton_kho = ? WHERE ma_thuoc = ?
                    """, (new_ton_kho, ma_thuoc))
                
                conn.commit()
                
                # Show success message with details of what was exported
                exported_count = len(updates)
                msg = f"Đã xuất thành công {exported_count} loại thuốc"
                if insufficient:
