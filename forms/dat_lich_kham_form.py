...
class DatLichKhamForm(QWidget):
    ...
    def init_ui(self):
        ...
        self.combo_phong_kham = QComboBox()
        self.combo_phong_kham.setMinimumHeight(40)
        self.combo_phong_kham.setStyleSheet("font-size: 18px;")
        self.combo_phong_kham.addItem("Nhấn để chọn phòng khám")
        ...
        form_layout.addRow("Phòng khám:", self.combo_phong_kham)
    ...
    def load_doctors_and_rooms(self):
        ...
        try:
            ...
            self.combo_phong_kham.clear()
            self.combo_phong_kham.addItem("Nhấn để chọn phòng khám")
            self.combo_phong_kham.addItems(rooms)
        finally:
            conn.close()
    ...
    def load_history_data(self):
        ...
        cur = conn.cursor()
        try:
            cur.execute("SELECT id, ho_ten, ngay_gio, bac_si, phong_kham, ghi_chu FROM lich_hen ORDER BY id ASC")
            rows = cur.fetchall()
        finally:
            conn.close()
        self.table_history.setRowCount(0)
    ...
    def save_appointment(self):
        ...
        phong_kham = self.combo_phong_kham.currentText().strip()
        ghi_chu = self.input_ghi_chu.text().strip()
        if not ho_ten or not ngay_gio or bac_si == "Nhấn để chọn bác sĩ" or phong_kham == "Nhấn để chọn phòng khám":
            QMessageBox.warning(self, "Thiếu thông tin", "Vui lòng nhập đầy đủ thông tin và chọn bác sĩ, phòng khám!")
            return
        try:
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("INSERT INTO lich_hen (ho_ten, ngay_gio, bac_si, phong_kham, ghi_chu, trang_thai) VALUES (?, ?, ?, ?, ?, ?)",
                        (ho_ten, ngay_gio, bac_si, phong_kham, ghi_chu, "Chưa duyệt"))
            conn.commit()
            conn.close()
            ...
            self.combo_phong_kham.setCurrentIndex(0)
